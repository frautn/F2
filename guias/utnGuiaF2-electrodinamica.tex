\documentclass[a4paper,11pt,dvipsnames, twocolumn]{article}
\usepackage[a4paper,left=2cm,right=2cm,top=2.5cm,bottom=2.5cm]{geometry}

\setlength{\columnseprule}{0.4pt}
\setlength{\columnsep}{20pt}

\usepackage[utf8]{inputenc}
\usepackage[spanish]{babel}
\usepackage{amsmath,amsthm,amssymb,amsfonts}
\usepackage{enumerate}
\usepackage{caption}
\usepackage{graphicx}
\usepackage{xcolor}
\usepackage{tabularx}

\usepackage[locale=FR, per-mode=fraction, separate-uncertainty=true]{siunitx}
\usepackage{physics}
\ExplSyntaxOn
\msg_redirect_name:nnn { siunitx } { physics-pkg } { none }
\ExplSyntaxOff

\usepackage{stackengine}  % Needed for colorsection.
\usepackage[most]{tcolorbox}  % Needed for colorsection.
\usepackage[lastexercise,answerdelayed]{exercise}
\usepackage{multicol}  % Respuestas en dos columnas.

\usepackage{circuitikz,pgfplots}  %pgfplots needed for axis environment.
\usepackage{tikz-3dplot}
\usetikzlibrary{babel}  % Solves the problems produced by changes to category codes made by some babel modules.
\usetikzlibrary{decorations, decorations.pathmorphing}
\usetikzlibrary{shapes.geometric}
\usetikzlibrary{arrows, arrows.meta}
\usetikzlibrary{calc}
\usetikzlibrary{patterns}
\usetikzlibrary{3d}  % Babinet.

\input{./guias_newcommands.code}

% \includeonly{termo_portada.code, termo_dilatacion.code, termo_calorimetria.code,
%   termo_preguntas_calorimetria.code, termo_transmision.code,
%   termo_preguntas_transmision.code, termo_primerppio.code,
%   termo_segundoppio.code, termo_preguntas_principios.code,
%   termo_adicionales.code}


\graphicspath{{./electrodinamica/img/}}

\pgfplotsset{compat=1.18}

\setlength{\parindent}{0pt}

\begin{document}

  \setcounter{section}{6}

  % \include{campos/campos_portada.code}
  % \include{campos/campos_electrico_puntuales.code}
  % \include{campos/campos_electrico_continuas.code}
  % \include{campos/campos_ley_gauss.code}
  % \include{campos/campos_potencial.code}
  % \include{campos/campos_magnetico_fuerza.code}
  % \include{campos/campos_magnetico.code}
  % \include{campos/campos_preguntas.code}

  \input{electrodinamica/electrodinamica_portada.code}
  % \definecolor{topcolor}{RGB}{0,121,138}
  % \renewcommand{\colorofsection}{topcolor}

  \input{electrodinamica/electrodinamica_circuitos.code}
  \input{electrodinamica/electrodinamica_induccion.code}
  % \input{electrodinamica/electrodinamica_transitorios.code}
  % \input{electrodinamica/electrodinamica_preguntas.code}

  \refstepcounter{section}%
  \addcontentsline{toc}{section}{\thesection\hspace{5pt}Transitorios en circuitos}
  \refstepcounter{section}%
  \addcontentsline{toc}{section}{\thesection\hspace{5pt}Preguntas sobre electrodinámica para el análisis}

% \twocolumn[\colorsection{Respuestas de la unidad II}]
%   \begin{multicols*}{2}
    % \shipoutAnswer
%   \end{multicols*}

  \twocolumn[\colorsectionnonumber{Respuestas de la unidad II}]
    \shipoutAnswer


\end{document}