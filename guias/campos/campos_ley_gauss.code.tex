...
\end{center}
%
\begin{Exercise}\label{p:gauss02a}
	¿Cuánto vale el flujo de campo eléctrico sobre la superficie esférica $S$ que encierra al dipolo eléctrico mostrado en la figura \ref{f:gauss02a}?
\end{Exercise}
\begin{Answer}
...
\begin{center}
	\begin{tikzpicture}[scale=0.5]
		\fill [blue!50](-1,0) circle(6pt) node[black, above left] {$-q$};
		\fill [red!50](1,0) circle(6pt) node[black, below right] {$q$};
		\draw [dashed](0,0) circle(70pt);
		\draw [] (1.2,-1.8) node[black, below right] {$S$};
	\end{tikzpicture}
	\captionof{figure}{Problema \ref{p:gauss02a}\label{f:gauss02a}}
\end{center}
%
...
\begin{Exercise}\label{p:gauss04}
	Se tiene un campo eléctrico 
  \begin{align*}
	\va*{E} = b \left [ (x+2y)\,\vu{i}+(2x+y)\,\vu{j} \right ] \, ,
  \end{align*}
