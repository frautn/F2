...
%
\begin{Exercise}
  En una región del espacio, el campo eléctrico está dado por la siguiente expresión:
  \begin{align*}
    \va*{E}(x,y,z) &= 3x^2y\,\vu{i} + (x^3-2yz)\,\vu{j} -y^2\,\vu{k}
  \end{align*}
  donde $x$, $y$ y $z$ están en metros y el resultado se obtiene en $\text{N/C}$. Dos cargas puntuales están ubicadas en las siguientes posiciones:
  \begin{itemize}
    \item Carga $A$:\par $q_A = \SI{3.0}{\micro\coulomb}$ en $\va*{r}_A = (1.0;2.0;0.0)\,\text{m}$
    \item Carga $B$:\par $q_B = \SI{-2.0}{\micro\coulomb}$ en $\va*{r}_B = (3.0;1.0;-1.0)\,\text{m}$
  \end{itemize}
  \textit{a}) Obtener el vector campo eléctrico en la posición de cada una de las partículas, solo debido al campo eléctrico externo (sin tener en cuenta el campo producido por las partículas puntuales).\par\medskip
  \textit{b}) Calcular el módulo de la fuerza \textbf{neta} ejercida sobre la partícula $A$.\par\medskip
  \textit{c}) Calcular el módulo de la fuerza \textbf{neta} ejercida sobre la partícula $B$.
\end{Exercise}
\begin{Answer}
  \begin{minipage}[t]{.4\textwidth}
    \textit{a}) $\va*{E}_A = (6\,\vu{i} + 1\,\vu{j} -4\,\vu{k})\text{ N/C}$;\\ $\va*{E}_B = (27\,\vu{i} + 29\,\vu{j} -1\,\vu{k})\text{ N/C}$\\ \textit{b}) $|\va*{F}_A| = \SI{15}{\micro\newton}$ \\ \textit{c}) $|\va*{F}_B| = \SI{77}{\micro\newton}$
  \end{minipage}
\end{Answer}
%
