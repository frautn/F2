\twocolumn[\colorsection{Circuitos de corriente continua}]
\setcounter{figure}{0}
%
\begin{Exercise}
...
\begin{center}
  ...
  \end{circuitikz}
  \captionof{figure}{Problema \ref{p:circuitos11}\label{f:circuitos11}}
\end{center}
%
\begin{Exercise}\label{p:circuitos21}
Calcular los valores de $R_1$ y $R_2$ del circuito mostrado en la figura \ref{f:circuitos21} a fin de que por la resistencia $R_0$ no circule corriente sabiendo que $R_1 + R_2 = 90\,\Omega$.
\end{Exercise}
\begin{Answer}
	\begin{minipage}[t]{.4\textwidth}
    $R_1 = \SI{37.5}{\ohm}$ y $R_2 = \SI{52.5}{\ohm}$
  \end{minipage}
\end{Answer}
%
\begin{center}
    \begin{circuitikz}[scale=1]
      \draw (0,0) to[battery2, invert, l=$6\,\text{V}$, color=cyan] (0,2)
      to[battery2, invert, l=$4\,\text{V}$, color=cyan] (0,4)
      to[R, l_=$30\,\Omega$, color=red] (3.5,4) to (5.5,4)
      to[battery2, l=$6\,\text{V}$, color=cyan] (5.5,0)
      to (3.5,0)
      to[R, l_=$50\,\Omega$, color=red] (0,0);
      \draw (0,2) to[R, l=\mbox{$R_0 = 50\,\Omega$}, color=red] (3.5,2);
      \draw (3.5,4) to[R, l=$R_1$, color=red] (3.5,2)
      to[R, l=$R_2$, color=red] (3.5,0);
    \end{circuitikz}
  \captionof{figure}{Problema \ref{p:circuitos21}\label{f:circuitos21}}
\end{center}
%
\begin{Exercise}
  Un resistor $R_1$ consume una potencia eléctrica $P_1$ cuando se conecta a una fem $\varepsilon$. Cuando el resistor $R_2$ se conecta a la misma fem, consume una potencia eléctrica $P_2$. En términos de $P_1$ y $P_2$: \textit{a}) ¿Cuál es la potencia eléctrica total consumida cuando los dos están conectados a esta fuente fem en paralelo? \textit{b}) ¿Y cuándo están conectados en serie?
\end{Exercise}
\begin{Answer}
	\begin{minipage}[t]{.4\textwidth}
