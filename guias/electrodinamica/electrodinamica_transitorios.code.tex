...
\setcounter{figure}{0}
%
\textcolor{red}{Agregar ejercicios de capacitores.}
...
\end{center}
%
\begin{Exercise}\label{p:transitorios10}El circuito de la figura \ref{f:transitorios10} se encuentra funcionando en régimen estacionario, con la llave cerrada. La capacidad del capacitor es $C = \SI{3.5}{\milli\farad}$, el valor de las resistencias es $R_1 = R_2 = R_3 = \SI{250}{\ohm}$, y la diferencia de potencial en la batería es $\varepsilon = \SI{32}{\volt}$. Si en un cierto instante se abre la llave, calcule la corriente que circulará a través de $R_3$, $\SI{1}{\second}$ después de abrir la llave, e indique el sentido de circulación de esa corriente. 
\end{Exercise}
\begin{Answer}
    \begin{minipage}[t]{.4\textwidth}
        $\SI{13.6}{\milli\ampere}$ en sentido antihorario.
    \end{minipage}
\end{Answer}
%
\begin{center}
  \begin{circuitikz}[scale=1]
      ...
      \draw (1.5,0) -- (2,0) to[battery2, l=$\varepsilon$, color=cyan] (3.5,0)
      to[R=$R_2$, color=red] (5,0) -- (5.5,0) -- (5.5,1.5);
      \draw (0,0.75) to[switch] (1.5,0.75);
      \draw (5.5,0.75) -- (6,0.75);
      \draw (0,-1.25) -- (0,2.75)
      to[C, l=$C$] (6,2.75) -- (6,-1.25);
      \draw (1.5,0) -- (1.5,1.5) to[R=$R_1$, color=red] (5.5,1.5); 
      \draw (0,-1.25) to[R=$R_3$, color=red] (6,-1.25); 
  \end{circuitikz}
  \captionof{figure}{Problema \ref{p:transitorios10}\label{f:transitorios10}}
\end{center}
%  
\begin{Exercise}\label{p:transitorios11} Considerando que el capacitor en el circuito mostrado en la figura se encuentra inicialmente descargado, que las resistencias son idénticas y que en el instante inicial se cierra la llave, seleccione la opción que mejor describe el valor de la corriente $i$ una vez que se alcanza el régimen estacionario, comparado con su valor inicial (inmediatamente después de cerrar la llave).\par\medskip
% \begin{tabularx}{\textwidth}{XX}
$\square$ 25\% menor que la corriente inicial.\\
$\square$ 25\% mayor que la corriente inicial.\\
$\square$ $33.3$\% menor que la corriente inicial.\\
$\square$ $33.3$\% mayor que la corriente inicial.\\
$\square$ $50$\% menor que la corriente inicial.\\
$\square$ $50$\% mayor que la corriente inicial.\\
$\square$ No circula corriente en el régimen estacionario.\\
% \end{tabularx}  
\end{Exercise}
\begin{Answer}
    \begin{minipage}[t]{.4\textwidth}
...
\begin{center}
  \begin{circuitikz}[scale=1]
    \draw [] (0,0) to[R=$R$, color=red] (0,-1.8) to[R=$R$, color=red] (2,-1.8);
    \draw [] (0,0) to[battery2, l=$\varepsilon$, invert, color=cyan] (2,0) to[switch]  (4.5,0) -- (4.5,-1.8) -- (4,-1.8);
    \draw [] (2,-1.3) to[R=$R$, color=red] (4,-1.3) -- (4,-2.3) to[C, l=$C$] (2,-2.3) -- (2,-1.3);
    \draw [-Stealth] (-0.4, -1.5) -- (-0.4, -0.2) node [midway, left] {$i$};
  \end{circuitikz}
  \captionof{figure}{Problema \ref{p:transitorios11}\label{f:transitorios11}}
\end{center}
